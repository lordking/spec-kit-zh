...
工作流程从一个想法开始——通常模糊且不完整。通过与AI的迭代对话，这个想法变成全面的PRD。AI提出需要澄清的问题，识别边界情况，并帮助定义精确的验收标准。在传统开发中可能需要数天会议和文档的工作，在数小时的专注规格中完成。这改变了传统的SDLC——需求和设计成为持续活动而不是离散阶段。这支持**团队流程**，团队审查的规格被表达和版本化，在分支中创建并合并。
