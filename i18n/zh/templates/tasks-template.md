...
**先决条件**：plan.md（必需）、spec.md（用于用户故事，必需）、research.md、data-model.md、contracts/

**测试**：下面的示例包含了测试任务。测试是可选的 - 仅在功能规格中明确要求时包含。

**组织**：任务按用户故事分组，以实现每个故事可独立实施与测试。
...
- **单项目结构**：位于仓库根目录下的 `src/`、`tests/`
- **Web应用**：`backend/src/`、`frontend/src/`
- **移动应用**：`api/src/`、`ios/src/` 或 `android/src/`
- 下方显示的路径假设为单项目结构——请根据 plan.md 中的结构进行调整

<!--
  ============================================================================
  重要：下面的任务仅用于说明用途的示例任务。

  `/speckit.tasks` 命令必须根据以下内容替换为示例任务：
...
- [ ] TXXX [P] 在 docs/ 中的文档更新
- [ ] TXXX 代码清理和重构
- [ ] TXXX 覆盖所有故事的性能优化
- [ ] TXXX [P] 在 tests/unit/ 中额外的单元测试（如要求）
- [ ] TXXX 安全加固
...
- **设置（阶段 1）**：无依赖 - 可立即开始
- **基础（阶段 2）**：依赖于设置阶段完成 - 阻塞所有的用户故事
- **用户故事（阶段 3+）**：所有依赖于基础阶段完成
  - 然后用户故事可以并行进行（如有人员配置）
...
- **用户故事 1（P1）**：可在基础阶段（（阶段 2））完成后开始- 不依赖其他故事
- **用户故事 2（P2）**：可在基础阶段（（阶段 2））完成后开始- 可能与 US1 集成但应可独立测试
- **用户故事 3（P3）**：可在基础阶段（（阶段 2））完成后开始- 可能与 US1/US2 集成但应可独立测试

### 每个用户故事内

- 测试（如包含）必须在实施前编写并确保失败
- 模型先于服务
- 服务先于端点
- 核心实施先于集成
- 在完成一个故事后再进入下一个优先级的故事
...
- 用户故事中所有标注 [P] 的测试任务可以并行运行
- 用户故事中标注 [P] 的模型任务可以并行运行
