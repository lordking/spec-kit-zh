...
在触发消息中，用户在 /speckit.specify 后输入的文本**即为**功能描述。请始终默认在此对话中你已拥有该描述，即使下方字面出现了 `{ARGS}`。除非用户提供的是空命令，否则不要要求其重复描述。
...
2. **在创建新分支前检查现有分支**：
   ...
   d. 使用计算后的编号和短名称（short-name）运行脚本 `{SCRIPT}`：
   ...
   **重要**：
   ...
   - 对于像"I'm Groot"这样的参数中的单引号使用转义语法，例如 "I'm Groot"转换为'I'\''m Groot'（或者如果可能，也可以使用双引号："I'm Groot"）
...
4. 遵循以下执行流程：
    ...
    5. 生成功能需求
       每个需求必须是可测试的
       对未明确的细节使用合理的默认值（在假设章节中记录这个假设）
    6. 定义成功标准
       创建可衡量且技术无关的结果。
       包括定量指标（时间、性能、数量）和定性衡量（用户满意度、任务完成率）
       每个标准必须在不依赖实现细节的情况下可验证
    7. 识别关键实体（如果涉及数据）
    8. 返回：成功（为计划准备的规格已经就绪）

5. 使用模板结构将规格写入 SPEC_FILE，用功能描述（参数）推导的具体细节替换占位符，同时保留章节顺序和标题层级结构。

6. **规格质量验证**：编写完成初始的规格后，根据质量标准对其进行验证：

   a. **创建规格质量检查清单**：在 `FEATURE_DIR/checklists/requirements.md` 生成检查清单文件，使用包含如下验证条目的检查清单模板结构：
      ...
      **功能**：[spec.md 的链接]
      ...
      - 标记为不完整的条目需要更新规格，然后才能执行 `/speckit.clarify` 或 `/speckit.plan`
      ```
   ...
   d. **更新检查清单**：每次验证迭代后，用当前的通过或失败状态更新检查清单文件

7. 完成报告，包含分支名称、规格文件路径、检查清单结果以及下一阶段（`/speckit.clarify` 或 `/speckit.plan`）的就绪性。

**注意**：脚本创建并检出新分支，并在写入前初始化规格文件。
...
6. **需要澄清的常见范围**（仅当在没有合理的默认值时）：
   - 功能范围和边界（包含/排除特定的用例）
   - 用户类型和权限（如果存在多种可能的冲突解释）
   - 安全/合规要求（当有法律/财务影响重大时）
...
2. **技术无关性**：不提及技术框架、编程语言、数据库或工具
3. **用户视角**：从用户/业务角度描述结果，而非系统内部
...
- "React 组件高效渲染"（具体的技术框架）
- "Redis 缓存命中率超过 80%"（具体的技术细节）
