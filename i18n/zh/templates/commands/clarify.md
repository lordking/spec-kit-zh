...
目标：检测并减少活动功能规格中的歧义或缺失决策点，并直接将澄清记录到规格文件中。
...
2. 加载当前规格文件。使用此分类执行结构化的歧义和覆盖率扫描。对于每个类别，标记状态：清晰/部分/缺失。生成用于优先级排序的内部覆盖率地图（如果没有问题将被询问，则不要输出原始地图）。
...
5. 在接受每个答案后进行整合（增量更新方法）：
    ...
    - 对于本次会议中的第一个整合答案：
       - 确保存在 `## Clarifications` 部分（如果不存在，请根据规格将其创建在最高级别上下文/概述部分之后）。
