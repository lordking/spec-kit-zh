...
1. **设置**：从仓库根目录运行 `{SCRIPT}` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对的。对于参数中的单引号，如 "I'm Groot"，使用转义语法：例如 'I'\''m Groot'（尽可能使用双引号："I'm Groot"）。
...
2. **来自契约**：
   ...
   - 如果被请求：每个契约 → 在故事阶段中在实现之前进行契约测试任务 [P]
