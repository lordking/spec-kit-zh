...
3. 起草更新的宪章内容：
   ...
   - 确保每个原则部分：简洁的名称行、段落（或项目符号列表）捕获不可协商的规则。若规则不明确则需明确说明理由。
   - 确保治理部分列出修正程序、版本控制策略和合规审查期望。

4. 一致性传播检查清单（将之前的检查清单转换为主动验证）：
   - 读取 `/templates/plan-template.md` ，确保"宪章检查"的任何内容或规则与更新后的原则对齐。
   - 读取 `/templates/spec-template.md` ，确保范围/需求对齐 - 如果宪章添加/删除了强制性的部分或约束，则更新。
   - 读取 `/templates/tasks-template.md` ，确保任务分类反映新增的或删除的原则驱动任务类型（例如，可观测性、版本控制、测试纪律）。
   - 读取 `/templates/commands/*.md` 中的每个命令文件（包括此文件），验证在需要通用指导时没有过时的引用（仅限于特定的代理名称如 CLAUDE）。
   - 读取任何运行时指导文档（例如，`README.md`、`docs/quickstart.md`，如果存在代理特定指导文件）。更新对更改原则的引用。
