---
description: 通过提出最多 5 个高度针对性的澄清问题，识别当前功能规范中说明不足的区域，并将答案编码回规范。
handoffs:
  - label: 构建技术计划
    agent: speckit.plan
...
2. 加载当前规范文件。使用此分类执行结构化的歧义和覆盖率扫描。对于每个类别，标记状态：清晰/部分/缺失。生成用于优先级排序的内部覆盖率地图（如果没有问题将被询问，则不要输出原始地图）。
...
5. 在接受每个答案后进行整合（增量更新方法）：
    ...
    - 对于本次会议中的第一个整合答案：
       - 确保存在 `## Clarifications` 部分（如果不存在，请根据规范将其创建在最高级别上下文/概述部分之后）。
       - 在其下，如果不存在，则创建今天的 `### Session YYYY-MM-DD` 子标题。
    ...
    - 然后立即将澄清应用到最合适的部分：
       ...
       - 术语冲突 → 跨规范规范术语；仅在必要时通过添加一次`(以前称为"X")`来保留原始。
    - 如果澄清使之前的模糊声明无效，则替换该声明而不是复制；不留任何过时的矛盾文本。
    - 在每次整合后保存规范文件，以最小化上下文丢失的风险（原子覆盖）。
